...
static std::string RegisterName(neocode_variable &Var, int UseRaw = 0) {
  ...
  if (Var.Name.size() && !UseRaw) return Var.Name + Swizz;
...
static std::string RegisterName(int Register) {
  if (Register < 0x10) return std::string("o") + std::to_string(Register);
  if (Register < 0x20)
  ...
  if (ASTNode->Type == ast_node::FUNCTION_CALL) {
    if (ASTNode->Id.compare("asm") == 0) {
      ...
      auto GetNextFromTokenSpecifier = [&LexerState, &Function, &ASTNode,
        ...
        if (Token.Type == ',') Token = LexerGetToken(&LexerState);
      ...
      Function->Instructions.push_back(In);
      return In;
    } else if (parser::IsTypeSpecifier(
                   SymbolTable->Lookup(ASTNode->Id)->SymbolType)) {
      ...
      if (FuncDef->SymbolType == 0) return neocode_instruction();
...
neocode_function cg_neo::BuildFunction(neocode_program *Program,
                                       ast_node *ASTNode) {
...
neocode_program CGNeoBuildProgramInstance(ast_node *ASTNode, symtable *S) {
  ...
  Program.Globals.push_back(neocode_variable(
      "NS_Position", "vec4", ast_node::STRUCT, Program.Registers.AllocOutput(),
  ...
  /*Program.Globals.push_back((neocode_variable){"NS_Normal",
                                               ...
                                               0});*/
  Program.Globals.push_back(neocode_variable(
      "NS_Color", "vec4", ast_node::STRUCT, Program.Registers.AllocOutput(),
      neocode_variable::OUTPUT_COLOR));
  Program.Globals.push_back(neocode_variable(
      "NS_Texture0", "vec4", ast_node::STRUCT, Program.Registers.AllocOutput(),
